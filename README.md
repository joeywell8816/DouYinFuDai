抖音福袋｜永久免费。本项目为个人业余时间开发的小工具，如觉得有价值，欢迎点 Star 支持。支持比特浏览器；支持账号分组策略（如运行一天、休息两天）；单机账号登录数量不设上限（建议 20–40 个以保证稳定性）；采集端与领取端在同一台电脑上运行；使用 Python 编写，启动速度相对较慢；中文界面，操作简单直观，不提供使用教学，功能更新与优化视个人时间与精力而定。
